...
class UNet(nn.Module):
    ...
    def forward(self, x, label, time):
        ...
        # first conv
        x = self.conv_in(x)
        # time embedding has shape (time_dim)
        if self.time_embed_pe:
            t = self.time_embd(time)
        else:
            t = self.time_embd(time[:, None])
        # label embedding has shape (B, 1, context_dim)
        ...
        for resblock, transformer, downsample in self.encoder:
            for i, rb in enumerate(resblock):
                x = rb(x, t)  # ResBlock

                # (resblock -> transformer) pattern
                if transformer is not None and i < len(transformer):
                    x = transformer[i](x, c)
        ...
        """
        # decoder
        for idx, (resblock, transformer, upsample) in enumerate(self.decoder):
            # concat with the previous one through -(idx + 1)
            # x has shape (B, C, H, W)
            x = torch.concat((x, encoder_output[-(idx + 1)]), dim = 1)
