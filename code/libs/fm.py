...
class FM(nn.Module):
    ...
    def compute_loss(self, x_start, label, noise=None):
        ...
        B = x_start.shape[0]
        t = torch.rand_like(label, dtype=torch.float, device=label.device)
        noise = torch.randn_like(x_start)

        x_t = x_start * t[:, None, None, None] + noise * (1 - t[:, None, None, None])

        loss = F.mse_loss(self.model(x_t, label, t), x_start - noise)
